...
export const userDatasDelete = functions.auth.user().onDelete(async (user) => {
  ...
  //----------------------------------
  // UserRef
  const profilesRef = admin.firestore().doc(`profiles/v1`).collection('users').doc(user.uid)

  // SocialRef
  const socialsRef = admin.firestore().doc(`socials/v1`).collection('users').doc(user.uid)
  ...
  const otherUsersFollowersDocs = await admin.firestore().collectionGroup('followers').where('followersId', '==', user.uid).get()

  // otherUsersFollowingsDocs
  const otherUsersFollowingsDocs = await admin.firestore().collectionGroup('followings').where('followingsId', '==', user.uid).get()

  // PostsDocs
  const postsDocs = await admin.firestore().doc('posts/v1').collection('users').where('authorId', '==', user.uid).get()
  //----------------------------------
  // バッチ処理で一括削除する
  //----------------------------------
